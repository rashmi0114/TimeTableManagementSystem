...
namespace Time_Table_Management_System.Tags
{
...

    {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                subName.Focus();
                errorManageTags.SetError(subName, " Enter Subject Name");
            }
            else if (subCode.Text == String.Empty)
            {
                subCode.Focus();
                errorManageTags.SetError(subCode, " Enter Subject Code");
            }
            else if (comboBoxRelatedTag.SelectedIndex == -1)
            {
                comboBoxRelatedTag.Focus();
                errorManageTags.SetError(comboBoxRelatedTag, " Select Related Tag");
            }
            else
            {
                Tag tag = new Tag();
                ...
                {
                    ErrorMessage ec = new ErrorMessage("there is somethin wrong when tag updating");
                    ec.Show();
                }
            }
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                SuccessMessage sm = new SuccessMessage("Tag deleted Successfully");
                sm.Show();
                dataGridView1.Rows.Clear();
                populateData();
                clear();
            ...
            {
                ErrorMessage em = new ErrorMessage("there is somethin wrong when tag deleting");
                em.Show();
            }
        }
